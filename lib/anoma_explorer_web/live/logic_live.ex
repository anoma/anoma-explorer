    ...
    <div class="stat-card mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <div
            class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
            title="Determined by index parity: even indices are nullifiers (consumed), odd indices are commitments (created)"
          >
            Resource Type
          </div>
          <div>
        ...
        <div>
          <div
            class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
            title="Position of this logic input within the action's logic inputs array"
          >
            Index
          </div>
        ...
        <%= if @logic["logicRef"] do %>
          <div class="md:col-span-2">
            <div
              class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
              title="Reference to the logic circuit (verifying key) that validates this resource's creation or consumption"
            >
              Logic Ref
            </div>
            <div class="flex items-center gap-2">
        ...
        <%= if @logic["action"] do %>
          <div>
            <div
              class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
              title="Blockchain network where this logic input was recorded"
            >
              Network
            </div>
            <div>
          ...
          <div>
            <div
              class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
              title="Blockchain block number where this logic input was included"
            >
              Block Number
            </div>
            <div class="flex items-center gap-2">
          ...
          <div>
            <div
              class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
              title="When this logic input was recorded on the blockchain"
            >
              Timestamp
            </div>
          ...
          <div>
            <div
              class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
              title="Parent action containing this logic input - identified by action tree root"
            >
              Action
            </div>
          ...
          <%= if @logic["action"]["transaction"] do %>
            <div>
              <div
                class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
                title="EVM transaction that submitted this action to the blockchain"
              >
                Transaction
              </div>
              <div class="flex items-center gap-1">
        ...
        <%= if @logic["resource"] do %>
          <div>
            <div
              class="text-xs text-base-content/60 uppercase tracking-wide mb-1"
              title="Link to the associated resource record with full details"
            >
              Resource
            </div>
            <div class="flex items-center gap-1">
  ...
  defp payloads_section(assigns) do
    ...
    <div class="stat-card mb-6">
      <h2
        ...
        title="Count of different payload types associated with this logic input"
      >
        Payload Counts
      </h2>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div
          class="bg-base-200/50 rounded-lg p-4 text-center"
          title="App-specific data for the logic circuit to process"
        >
        ...
        <div
          class="bg-base-200/50 rounded-lg p-4 text-center"
          title="Data for resource indexing and discovery by other parties"
        >
        ...
        <div
          class="bg-base-200/50 rounded-lg p-4 text-center"
          title="Data stored off-chain, referenced by this logic input"
        >
      ...
      <div class="stat-card">
        <div class="flex items-center justify-between mb-4">
          <h2
            ...
            title="Zero-knowledge proof validating that this logic input satisfies the logic circuit constraints"
          >
            Proof
          </h2>
